...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match = "//*[local-name() = 'import_title']">
        ...
        <!-- If the title.xml file is in tag_gen, import the title format xsl (html_title.xsl) -->
        <!-- If the file isn't available, it will replace the tag in html_base with nothing -->
        <xsl:if test="doc-available($filePath)">
            <axsl:include href="html_title.xsl"/>
        </xsl:if>
    </xsl:template>
</xsl:stylesheet>